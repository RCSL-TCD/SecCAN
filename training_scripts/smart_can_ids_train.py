...
class canTrainDataset(Dataset):
    def __init__(self):
        ...
        x_load = np.loadtxt('./sonata_train_x_uint8.txt',delimiter = ",",dtype=np.float32)#dos_fuzzy_train_x.txt
        y_load = np.loadtxt('./sonata_train_y.txt',delimiter = ",",dtype=np.float32)#dos_fuzzy_train_y.txt
...
class canValDataset(Dataset):
    def __init__(self):
        #will be mostly used for dataloading
        x_load = np.loadtxt('./sonata_val_x_uint8.txt',delimiter = ",",dtype=np.float32)#dos_fuzzy_val_x.txt
        y_load = np.loadtxt('./sonata_val_y.txt',delimiter = ",",dtype=np.float32)#dos_fuzzy_val_y.txt
        self.x = torch.from_numpy(x_load)
        self.y = torch.from_numpy(y_load)
        self.n_samples = x_load.shape[0]
...
val_batch_size = 1000
trainloader = DataLoader(dataset=trainDataset, batch_size=train_batch_size, shuffle=True)#We train the model by setting shuffle as true to make batch training stable.
...
a4 = 4
a5 = 4
directory = "./smart_can_4bit_200_sonata"
data_folder = "./smart_can_4bit_200_sonata/data"    
model_folder = "./smart_can_4bit_200_sonata/model" 
os.mkdir(directory)
...
for j in range(num_epochs):  
    ...
    #Compute the loss for the batch
        labels = labels.reshape([train_batch_size,1])
...
#Save the model for this particular epoch after training.
    ...
    for l,(val_inputs,val_labels) in enumerate(validationloader): #The validation loop for the models to be trained here.
        ...
        val_labels = val_labels.reshape([val_batch_size,1])
